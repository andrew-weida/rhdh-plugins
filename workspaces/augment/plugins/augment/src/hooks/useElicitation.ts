...
export function useElicitation({
  ...
  const respondedElicitationIdRef = useRef<string | null>(null);
  ...
  // Clear elicitation state when streaming phase leaves pending_elicitation
  useEffect(() => {
  ...
  // Detect new elicitation requests from streaming state
  useEffect(() => {
    if (
